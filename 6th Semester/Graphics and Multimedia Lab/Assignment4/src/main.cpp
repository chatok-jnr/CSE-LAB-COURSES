...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Md. Sakib Hosen", NULL, NULL);
    if (window == NULL) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (makeGreen) {
            ...
            glUniform4f(glGetUniformLocation(shaderProgram, "ourColor"), redValue, 0.0f, 0.0f, 1.0f);
        }
...
void processInput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS && !aKeyPressed) {
        aKeyPressed = true;
        makeGreen = !makeGreen;
    }
